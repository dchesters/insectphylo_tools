...
# 13 Jun 2023: dies if user gives same name for input and output file.
# 26 Sep 2023: User might want to get fasta entries corresponding to the terminals of a phylogeny,
# 		script will now extract such a terminal list if given -newick argument.
# 06 Jan 2024: Option to relabel where using Blastclust style filtered data.
# 
# 
# 
# 
...
if($print_current_entry == 1)
	{
	my $print_ID=$current_id;
	if($relabel == 1){$print_ID = "OTU_" . $current_id};
	print OUT1 ">$print_ID\n$line";	
	$fasta_ID_printed{$current_id} = 1
	};
...
if($arguments =~ /-newick\s+(\S+)/)
	...
	$newick_file = $1;
	###################################
	# pull out terminal IDs from newick file
	open(IN, $newick_file) || die "\nerror cant open file ($newick_file)\n";
	open(OUT, ">$newick_file.terminal_list") || die "\nerror cant open output\n";
	while(my $line = <IN>){if($line =~ /\(/){$tree = $line}};
	close IN;
	while($tree =~ s/([\(\,])([A-Z][^\:\,\)]+)([\:\,\)])/$1$3/)
		{
		my $terminal = $2;print OUT "$terminal\n";
		};
	##################################
	print "\nuser opted to extract list of terminals from newick, this can now be used in same script on fasta file.\nFIN\n\n";
	exit;
	};
...
if($database_file eq $out_file){die "\n\nerror, you have given same name for input and output.\n\n"};
